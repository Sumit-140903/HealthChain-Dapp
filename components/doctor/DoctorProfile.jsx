...
const ProfileImageUpload = ({ currentImage, onImageUpdate, loading }) => {
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      onImageUpdate(uploadResult.hash);
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      const result = await ipfsService.uploadJSONToIPFS(profileDataToUpload, {
        name: `doctor-metadata-${formData.name || "unknown"}`,
        type: "doctor-metadata",
      });

      setProfileData(profileDataToUpload);
      setDoctorData((prev) => (prev ? { ...prev, IPFS_URL: result.url } : prev));
      setIsEditing(false);
      toast.success("Profile updated locally. On-chain update not supported yet.");
    } catch (error) {
