...
class IPFSService {
  constructor() {
    this.pinataJWT = PINATA_JWT || null;
    this.gateway = PINATA_GATEWAY || "https://gateway.pinata.cloud";
  }

  async uploadToIPFS(file, metadata = {}) {
    if (!this.pinataJWT) {
      throw new Error("IPFS upload is not configured. Missing PINATA_JWT.");
    }
    try {
  ...
  async fetchFromIPFS(hash) {
    try {
      let response = await fetch(`${this.gateway}/ipfs/${hash}`);
      if (!response.ok) {
        // fallback to ipfs.io gateway
        response = await fetch(`https://ipfs.io/ipfs/${hash}`);
      }
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
